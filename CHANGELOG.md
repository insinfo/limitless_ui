...
- Fixed `li-datatable` handing the responsive expand/collapse control to the action column, which left a hit area a few pixels wide next to the row's buttons on a phone. The control now goes to the first visible column, skipping columns that opt out through the new `DatatableCol.responsiveControlEligible` (`false` on `DatatableActionColumn`). `DatatableSettings.responsiveControlColumnKey` still overrides it.
- Added `DatatableSettings.responsiveControlMode`. `DatatableResponsiveControlMode.column` renders a dedicated leading column for the expand control, ahead of the selection checkbox, so the toggle gets a finger-sized cell of its own; `inline` (the default) keeps the control in a data cell.
- Added `DatatableSettings.responsiveDetailsTrigger`, choosing whether only the control opens the details row (`control`, the default, leaving `onRowClick` free) or a click anywhere in a collapsed row opens it (`row`).
- Added `expandRowDetailsLabel` and `collapseRowDetailsLabel` inputs for the control's accessible name. The control cell is now keyboard reachable and toggles on Enter/Space.
- Fixed the grey block behind a fixed (sticky) datatable column on any surface other than a card — a modal, most visibly. The theme scopes `--card-bg` to its `.card` rule, so the old `var(--card-bg, var(--body-bg, #fff))` fell through to the page background. The datatable now resolves the background actually painted behind it and publishes it as `--li-datatable-sticky-bg`, which you can also set yourself.
- Fixed the datatable's column-visibility, export and row action menus running off a short viewport with no way to reach the entries outside it; they are now fitted to the space next to their trigger and scroll internally. The action menu also gained placement fallbacks and modal-aware stacking.
- Made the loading helpers' stacking configurable so an application can adopt them without touching call sites. `LiSimpleLoading.defaultBodyZIndex` and `defaultTargetZIndex` are mutable statics now (assign them once in `main()`), and `LiNarratedFullScreenLoading.defaultZIndex` follows `defaultBodyZIndex + 1` unless assigned, so it stays above the plain curtain even when the app moves that value; `resetDefaultZIndex()` restores the derivation. `showOnBody`'s `zIndex` parameter became nullable, since a mutable static cannot be a default parameter value — passing an explicit value still works.
- Added `LiSimpleLoading.isVisible` and the opt-in `LiSimpleLoading.debugAssertSingleShow`. `show` has always called `hide` first, which is resource cleanup rather than idempotence: without it a second `show` overwrites the three stream subscriptions and the `ResizeObserver` without cancelling them, and the previous `_root` is left orphaned in the DOM with no reference left to remove it — a full-screen curtain stuck on the page. The cleanup stays; turning the flag on in `main()` additionally trips an assertion when showing over an already visible overlay, so a codebase whose convention is one `hide` per `show` sees the violation in development. Off by default, because re-showing is legitimate — two overlapping loads on the same table both call `show`, and `LiDataTableComponent.showLoading()` is exactly that.
- Fixed opening one overlay leaving other open overlays on screen — the datatable's column-visibility menu and a row's action menu was the reported pair, but every overlay in the library was affected. Outside-click detection ran in the bubble phase, so a trigger calling `stopPropagation()` on its own click was invisible to every other open overlay; the datatable's menus both do this, the action menu because the click must not become a row click. Detection now goes through a shared `listenOutsideClick` helper that subscribes in the capture phase, which runs before the target and is therefore not defeatable downstream. Applies to `li-select`, `li-multi-select` (via `liClickOutside`), `li-treeview-select`, `li-tag-filter`, `li-typeahead`, `li-dropdown-menu`, `liDropdown` and its submenus, `li-date-picker`, `li-date-range-picker`, `li-time-picker`, `li-color-picker`, `li-popover`, `li-tooltip`, `li-simple-popover`, the sweet-alert popover, and the datatable's own menus. Clicking an overlay's own trigger or panel behaves exactly as before — each handler already tested for that first.
- Fixed `li-time-picker`'s selection disc not sitting on the number it selects, most visibly on the 24-hour inner ring where it was about 10px out. The disc was placed with `translate(-50%, -100%)`, which puts it entirely past the hand's tip so its centre lands half a disc further out, and the hand length was a pair of magic numbers (`31`, `18`) unrelated to the ring radii. Compensating through the hand length cannot work: the hand is a percentage of the clock, which drops from 17rem to 15.5rem on phones, while the disc is a fixed rem size. The disc is now centred on the hand's tip and the hand is simply the ring's radius, so the two coincide at any clock size. Covered by a case that compares the disc's centre with the active label's centre in all four quadrants of both rings.
- Fixed the two rings of `li-time-picker`'s 24-hour clock nearly touching, and the click boundary between them not matching what is drawn. The inner ring (0 and 13–23) got the class `time-picker-dial-label-inner` but no CSS rule at all, so it rendered at the outer ring's size — 40px labels with 43px between the radii. The inner ring is now smaller and pulled further in, the way Material's clock does it, and the hit-test boundary is derived from the two radii instead of a standalone `0.72`, which sat past the midpoint and made clicks nearer the outer ring select an inner hour. Covered by two cases in `test/time_picker/li_time_picker_component_test.dart`.
- Fixed `constrainOverlayHeightToViewport` undoing its own cap one layout pass later, which affected `li-color-picker`, `li-date-picker`, `li-date-range-picker` and `li-time-picker` whenever the panel had no CSS-fixed height. It measured the already-capped height and concluded the panel fitted; it now measures the natural height.
- Fixed `li-dropdown-menu` with `closeOtherMenusOnOpen: false` closing a previously opened menu anyway. Coordinated-toolbar compositions relied on the trigger's `stopPropagation()` hiding the click from other menus; with capture-phase detection the click is now visible, so the outside-click handler skips clicks that land on another `li-dropdown-menu` trigger and leaves the decision to the menu being opened, which is where the flag lives.
- Documented all of the above under "Datatable responsivo, colunas fixadas e menus em `1.0.0-dev.40`" in `doc-pt_BR.md`/`doc.md`, and demonstrated it in the example app's datatable page.

## 1.0.0-dev.39

- Added a `refresh()` method to `li-datatable` for the case where the bound items are mutated in place instead of being replaced — the typical action column whose button flips a field of the row's own object, e.g. `orgao.ativo = !orgao.ativo`. The datatable renders from rows it built during the previous draw and caches them under a signature that only tracks the `DataFrame` identity, its length and the layout inputs, so a mutation on an instance changed nothing that the component could observe: the row kept the value read from `toMap()` at build time and the screen stayed stale. The workaround was to reassign the input (`table.data = items`) purely to invalidate that cache. `refresh()` bumps the cache revision and schedules the rebuild on the next animation frame; pass `refresh(immediate: true)` to rebuild synchronously, so `rows`/`renderedRows` already carry the new values when the call returns (writing them to the DOM still happens on the next change-detection pass either way). Row selection and responsive row expansion are carried across the rebuild, keyed the same way as virtual-scroll selection (`DatatableSettings.rowKeyResolver`), so toggling a field no longer clears the checkboxes — sorting, paging and the active search filter are untouched. The existing `update()` is now an alias for `refresh()` and keeps working. Covered by new cases in `test/datatable/li_datatable_component_test.dart`, including the case where the toggle is a button built in `customRenderHtml` with a native `onClick.listen` listener rather than a `DatatableAction` — the redrawn cell replaces the button element, and the listener bound to the new one keeps working. Demonstrated in the example app under the datatable page ("refresh(): redesenhar após mutar a instância"), which lists `Orgao implements SerializeBase` instances through a `DatatableFormat.bool` column plus a `customRenderHtml` enable/disable button, and lets you turn the `refresh()` call off to see the model and the screen drift apart. A second example page, "Lista de Órgãos" (`example/lib/src/pages/lista_orgao/`), replicates a full real-world list screen — header actions, a custom filter outside the datatable contract, server-side pagination returning a fresh `DataFrame` of fresh instances on every query, and row selection feeding a delete action — with a selector that switches the redraw strategy between `refresh()`, reassigning `data`, reloading from the service, and doing nothing.

## 1.0.0-dev.38

- Added a `contentClass` input to `li-wizard` and a `bodyClass` input to `li-wizard-step`, so the theme chrome around the steps can be turned off. Both classes come from the Limitless theme — `.wizard > .content` carries the `mb-3` spacing and `.content > .body` carries a horizontal padding — and neither could be removed from the outside: `.content` belongs to the wizard's own template, so a consumer's encapsulated stylesheet never reaches it, and the step's `body` class is a `@HostBinding` that is reapplied after the parent's change detection, defeating any `[class.body]="false"` written in the host template. That left overriding the theme with `!important` from a global stylesheet as the only way out. `contentClass` defaults to `'mb-3'` and `bodyClass` to `true`, so existing wizards are unaffected; set them to `''` and `false` when the wizard is rendered inside a container that already provides its own padding and footer spacing, such as a modal. The structural `content` class is always kept. Covered by `test/wizard/li_wizard_component_test.dart`.

## 1.0.0-dev.37

- Fixed `li-quill-text-editor` never rendering the initial `ngModel`/form value. The bridge passed the raw HTML string to Quill's `clipboard.convert`, which is the Quill 1.x signature; Quill 2.x destructures its first argument as `{ html, text }`, so a string yielded `html: undefined` and the call silently returned an empty delta, which `_applyPendingHtmlValue` then wrote into the editor. The interop now declares `convert` as taking a `ClipboardConvertInput({ html, text })` anonymous object and the bridge wraps the value accordingly, so the pending HTML written by `writeValue` before initialization survives into the created editor. Programmatic `writeValue` calls after initialization were equally affected and are fixed by the same change. Regression covered by `test/quill_text_editor/li_quill_text_editor_bridge_test.dart`, which runs the real bridge against a scripted `window.Quill` that destructures `{ html, text }` exactly like Quill 2.x.
- Fixed every toolbar control of a `li-quill-text-editor` created without `enableTableSupport` throwing `Cannot destructure property 'cellSelection' of 'this.getTableBetter(...)'` on click — and therefore doing nothing — whenever the quill-table-better script is loaded on the page. That script replaces Quill's global `modules/toolbar` with a subclass whose control listeners destructure `this.getTableBetter()` on every click/change, and `getTableBetter()` returns `quill.getModule('table-better')`, which is `undefined` for editors that did not enable the module. The bridge now patches `getTableBetter` on the registered toolbar prototype (once, marker-guarded) to fall back to an empty object, which is safe because every call site in quill-table-better already guards `cellSelection?.`/`selectedTds?.` before use; editors with table support resolve their real module exactly as before. Regression covered by `test/quill_text_editor/li_quill_text_editor_bridge_test.dart` (missing-module fallback, real-module passthrough, and one-time marker-guarded application).
- Added an `enableImages` input to `li-quill-text-editor`, `false` by default: out of the box, pasted or dropped image files are ignored (the editor is created with a no-op uploader handler) and `<img>` elements are stripped by a clipboard matcher, which covers pasted HTML as well as HTML written through `ngModel`/`writeValue`; set `[enableImages]="true"` to accept images. An empty uploader `mimetypes` whitelist would not work here: Quill expands module options with lodash `merge`, which merges arrays by index, so an empty list is silently discarded in favor of the default png/jpeg whitelist — replacing the handler function is what actually drops the files. Content already present in the editor is left untouched, and programmatic delta APIs such as `setDeltaJson` are intentionally not filtered. Toggling the input at runtime requires `enableDestructiveToolbarRebuild`, like `enableTableSupport`, since both uploader options and matchers are fixed at Quill construction. Covered by new cases in `test/quill_text_editor/li_quill_text_editor_component_test.dart`.

## 1.0.0-dev.36

- Added an `openChange` output (`Stream<bool>`, `true` on open and `false` on close) to `li-select`, `li-multi-select`, `li-treeview-select`, `li-tag-filter`, `li-datatable-select`, `li-date-picker`, `li-date-range-picker` and `li-time-picker`, matching the name `liDropdown` already used for its submenus. None of these had any way to observe that their dropdown/modal had opened, so consumers that wanted to load the option list on demand had no hook to load it from and had to fetch it upfront — on `ngOnInit` of a screen that may never open the field. Only real transitions are emitted: reopening an already open dropdown stays silent, as does closing an already closed one, which matters because paths like selecting an option or pressing Escape call `closeDropdown()` without knowing the current state. Nothing is emitted while the component is being destroyed, so a consumer holding the component through a `ViewChild` and subscribing to the stream directly does not receive a spurious `false` during teardown. `li-color-picker` already exposed this through its `pickerShow`/`pickerHide` outputs and was left alone. Covered by `test/select/li_select_open_change_test.dart`.
- Added a cancelable `beforeOpen` output (`Stream<LiBeforeOpenEvent>`) to `li-select` and `li-multi-select`, emitted right before the dropdown opens while it is still closed. Calling `preventDefault()` on the event keeps the dropdown closed and suppresses the matching `openChange`, so opening can be gated on a permission check, a confirmation, or a required field elsewhere in the form. It follows the cancelable-event shape `liNav` already used for `LiNavChangeEvent`, and like it the stream is synchronous, so `preventDefault()` must be called from the handler itself rather than after an `await`; to gate on asynchronous work, prevent the default and call `openDropdown()` when it resolves. Only a real open emits it, so an already open dropdown never re-emits. Covered by `test/select/li_select_before_open_test.dart` and `test/multi_select/li_multi_select_before_open_test.dart`.
- Fixed `li-multi-select` not emitting `openChange` when the dropdown was opened or closed from the trigger — the click binding, and therefore the common path. `toggleDropdown` assigned `dropdownOpen = !dropdownOpen` before delegating to `openDropdown`/`closeDropdown`, which set that same field themselves, so both read the already-updated value as their previous state and concluded nothing had changed. The redundant assignment was removed, making `toggleDropdown` match `li-select`. This also repairs two older casualties of the same line: closing through the trigger now marks the control as touched and restores focus to it, neither of which happened before, since both were guarded on the previous state that the flip had already destroyed.
- Added an `open` output to `li-modal`, mirroring the `close` it already had. Because `lazyContent` only creates the projected content once the modal is open, this is the earliest point at which a consumer can load data for that content; it also fires for a `[start-open]="true"` modal. Covered by `test/modal/li_modal_open_event_test.dart`.
- Added a `requestDataOnOpen` input to `li-datatable-select` that emits `dataRequest` with the current `dataTableFilter` the first time the modal opens. Its inner modal already used `lazyContent`, so the datatable was only created on open — but it never asks for data on its own, emitting `dataRequest` only when the user paginates, sorts or searches. A consumer that stopped preloading the list would therefore get an empty modal until the user typed something. Only the first open emits, so reopening keeps the data already loaded; use `openChange` to reload on every open. Covered by `test/datatable_select/li_datatable_select_open_test.dart`.

## 1.0.0-dev.35

- Fixed `li-tag-filter` dropping the initial `ngModel`/form selection when `[dataSource]` resolves asynchronously. `writeValue` did retain the bound value, but `_rebuildSelectionState` then overwrote it with the selection derived from `options` — empty while the tags were still loading — so by the time the tags arrived there was nothing left to replay. Deriving the bound value back from the options is now confined to selection changes, leaving a written value intact until the options it refers to exist. Besides showing no tags as selected, this also made the first tag the user clicked emit a model containing only that tag, silently discarding the original selection. Regression covered by `test/tag/li_tag_filter_initial_value_test.dart`.
- Fixed `li-typeahead` dropping the initial `ngModel`/form value when `[dataSource]` resolves asynchronously. `writeValue` matched the value against `_allItems` and, on a miss, kept only the raw value as display text without retaining it, while `_applyDataSource` tried to recover the selection from `_selectedItem` — which derives from those same empty items. The component now retains the last written value and resolves it once the items arrive. Previously the input showed the raw value (e.g. `42` instead of `John Doe`), never resolved, and — with the default `editable="false"` — was wiped to empty on the first blur. Regression covered by `test/typeahead/li_typeahead_initial_value_test.dart`.
- Bumped `popper` to `^1.3.0`, which fixes floating elements keeping their Popper-written inline styles after `dispose()`. This makes a `li-dropdown-menu` whose `container` flips from `body` to `inline` — as the PDF viewer does when entering fullscreen — render at its own CSS position instead of staying frozen at the viewport coordinates computed before the flip.
...
- Fixed `li-dropdown-menu` mobile sheet/modal presentations used together with `container="body"` (e.g. the SALI process header "Ações"/"Imprimir"/"Gerar" menus on phones): their layout rules were scoped under `:host`, so once the panel was moved into the body portal they stopped matching and the menu either showed only its backdrop or collapsed to a Bootstrap `dropdown-menu` stuck in the top-left corner. The mobile-presentation styles are now attribute-scoped and survive the portal, so the sheet/modal renders as a proper fixed panel. Regression covered by `test/dropdown_menu/li_dropdown_menu_body_mobile_test.dart`.
- Made the `li-dropdown-menu` mobile sheet/modal layout (position, width, centering) win over consumer utility classes passed through `menuClass` (e.g. Bootstrap `.w-100`, which is `width: 100% !important`); previously such classes stretched the panel and pushed it off-center on the x-axis.
- Improved the `li-dropdown-menu` mobile header close button (`.li-dropdown-menu__mobile-close`) styling so it renders as a clean icon button (appearance reset, no default button box/shadow, sized icon, `:focus-visible`-only outline) once the mobile sheet/modal is visible.
- Changed the `li-pdf-viewer` mobile actions menu so zoom is a single "Zoom" entry that opens a dedicated zoom submenu instead of inlining every zoom level, and presented the mobile actions/zoom menus as centered modals. The desktop toolbar zoom dropdown is unchanged.
...
- Fixed an intermittent `liForm` validation race where native `[liRequired]` feedback could be read before the `is-invalid` state was applied.
...
- Expanded automation hooks across general interactive components including dropdown menus, datatables, treeviews, accordions, breadcrumbs, pagination, tabs, toasts, tooltips, popovers, and SweetAlert overlays.
...
- Added custom trigger templates for select-like controls, including `liSelectTrigger`, `liMultiSelectTrigger`, `liDatatableSelectCustomTrigger`, `liDatePickerTrigger`, `liDateRangePickerTrigger`, and `liTimePickerTrigger`, so apps can render badges, chips, or toolbar buttons while keeping component-owned focus, disabled, automation-hook, and overlay-positioning behavior.
- Added predefined range presets to `li-date-range-picker` through `LiDateRangePreset` and `[presets]`, with optional custom-range reveal, immediate or Apply-button confirmation, `alwaysShowCalendars` for showing shortcuts beside the calendars, and `showCalendarsForCustomRange` for opening custom current values directly in calendar mode.
- Changed `li-date-picker`, `li-date-range-picker`, and `li-time-picker` to default to fullscreen `mobilePresentation="modal"` on mobile viewports, with stable viewport-sized layout and internal scrolling so selecting values no longer resizes or repositions the picker UI mid-interaction.
- Stabilized picker overlay layout by clearing stale viewport constraints before Popper remeasures desktop panels, and by keeping `li-date-picker` mobile calendar content height natural so scrolling happens on the fullscreen panel only when needed.
- Aligned `li-time-picker` footer styling with the date pickers by adding a full-width separator and themed footer background above the action buttons.
- Added the `liDatePickerTrigger` badge example to the date picker demo page and covered it in the real Puppeteer flow.
- Fixed fullscreen mobile `li-modal` sizing to use natural height over `min-height: 100dvh`, preventing projected datatables from visually overflowing the modal. `enableModalBodyClass=false` now leaves the body wrapper raw by default, while the opt-in `enableModalBodyLayout`/`bodyClass` API remains available for no-padding custom flex bodies.
- Added `mobileResponsiveCollapse` forwarding to `li-datatable-select` so mobile modal tables can use the datatable details-row layout without breaking row selection by touch/click.
- Added fullscreen mobile presentation to `li-treeview-select` and `selectDescendantsOnParentSelect` for selecting/deselecting loaded child branches from a parent node in multiple mode.
- Fixed multi-selection summary chips in `li-datatable-select` and `li-treeview-select` so the third and later selected labels are actually hidden before the `+N` counter, even when Bootstrap badge display utilities are applied.
- Added `wrapTokens` to `li-token-field` and documented `wrapSelectedBadges` for `li-tag-filter`, allowing token/tag inputs to grow vertically instead of forcing selected chips into a single horizontal row.
