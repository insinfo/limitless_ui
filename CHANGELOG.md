...
- Added an `enableImages` input to `li-quill-text-editor`, `false` by default: out of the box, pasted or dropped image files are ignored (the editor is created with an empty uploader mimetype whitelist) and `<img>` elements are stripped by a clipboard matcher, which covers pasted HTML as well as HTML written through `ngModel`/`writeValue`; set `[enableImages]="true"` to accept images. Content already present in the editor is left untouched, and programmatic delta APIs such as `setDeltaJson` are intentionally not filtered. Toggling the input at runtime requires `enableDestructiveToolbarRebuild`, like `enableTableSupport`, since both uploader options and matchers are fixed at Quill construction. Covered by new cases in `test/quill_text_editor/li_quill_text_editor_component_test.dart`.
