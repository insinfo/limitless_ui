...
<li-wizard
  ...
  String get subtitle => isPt
      ? 'Fluxos em etapas e formulários guiados'
      : 'Stepped flows and guided forms';
  ...
  String get basicSubmittedLabel =>
      isPt ? 'Formulário enviado!' : 'Form submitted!';

  String get validationStatusLabel =>
      isPt ? 'Status da validação:' : 'Validation status:';
  ...
  String wizardActionSummary(LiWizardActionsContext context) {
    if (context.isLastStep) {
      return isPt
          ? 'Última etapa pronta para envio'
  ...
  bool _validateStep(int stepIndex) {
    final isValid = switch (stepIndex) {
      0 => validationLocation.trim().isNotEmpty &&
          validationPosition.trim().isNotEmpty &&
          validationName.trim().isNotEmpty &&
          validationEmail.trim().isNotEmpty,
      1 => validationDegree.trim().isNotEmpty &&
          validationUniversity.trim().isNotEmpty,
      2 =>
        validationCompany.trim().isNotEmpty && validationRole.trim().isNotEmpty,
      3 => validationSource.trim().isNotEmpty &&
          validationAvailability.trim().isNotEmpty,
      _ => true,
    };
